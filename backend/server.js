...
app.post('/api/orders', async (req, res) => { 
    try { 
        ...
        const order = await Order.create({ ...req.body, status: 'استلام الطلب', time: now, timestamps: { received: now } }); 
...
app.put('/api/orders/:orderId/status', async (req, res) => { 
    try { 
        ...
        if(order) {
            ...
            if(req.body.status === 'قيد التجهيز') order.timestamps.preparing = now;
            if(req.body.status === 'الطلب جاهز') order.timestamps.ready = now;
            if(req.body.status === 'استلم الطلب') order.timestamps.delivered = now;
            order.markModified('timestamps');
            await order.save();
...
app.get('/api/admin/summary', async (req, res) => {
    try { const orders = await Order.find(); res.json({ totalOrders: orders.length, totalRevenue: orders.reduce((sum, o) => sum + o.total, 0), branchesCount: await Branch.countDocuments(), usersCount: await User.countDocuments() }); } catch(e) { res.json({ totalOrders: 0, totalRevenue: 0, branchesCount: 0, usersCount: 0 }); }
});
