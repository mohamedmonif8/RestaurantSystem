...
app.get('/', (req, res) => res.send('🚀 Abu Mahal Server is running perfectly!'));
...
mongoose.connect(dbURI)
    .then(() => { console.log('✅ Database Connected!'); initAdmin(); })
...
async function initAdmin() {
    try {
        if (!(await User.findOne({ username: 'admin' }))) {
            await User.create({ name: 'المدير العام', username: 'admin', password: '123', role: 'admin' });
        }
    } catch(e) {}
}
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (user) res.json({ success: true, user }); else res.json({ success: false, message: 'بيانات الدخول خاطئة' });
...
// مسارات الطلبات
app.post('/api/orders', async (req, res) => { try { const order = await Order.create({ ...req.body, status: 'استلام الطلب', time: new Date().toLocaleTimeString() }); res.json({ success: true, orderId: order.id }); } catch(e) { res.json({ success: false }); } });
...
app.get('/api/admin/summary', async (req, res) => {
    try {
        ...
        res.json({ totalOrders: orders.length, totalRevenue: orders.reduce((sum, o) => sum + o.total, 0), branchesCount: await Branch.countDocuments(), usersCount: await User.countDocuments() });
